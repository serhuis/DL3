...
u8	jp1_state = 1;			// 0 - JP1 Open, 1 - JP1 Close
...
void Timer_A0_SetDelay(u16 period) {
	...
	CCR0 	 = period - 1;      		// Period T(us) * F(MHz)
        CCR1 	 = 0;      		// Period T(us) * F(MHz)
	TACCTL0 = CCIE;						// Разрешаем прерывание таймера по достижению значения TACCCR0.
	//
	_BIS_SR(GIE);    					// Разрешаем прерывания
}
//--------------------------------------------------------------------------------
// Function		: void Timer_A0_StartPWM(u16 PulseWidth)
// Parameters	: PulseWidth in us (1..65535) - for 8 MHz DCO
// Return		: None
// Description	: Function initiates PWM 
//--------------------------------------------------------------------------------
#define PWM_PWRIOD      1024
void Timer_A0_StartPWM(u16 PulseWidth){	
  u16 tmp = PWM_PWRIOD/PulseWidth;
...
//--------------------------------------------------------------------------------
void main(void) {
	u8	byte, sh, len, irpulses = 3, fADCStarted = 0;
	int i;
	u8  loc_buf[2];
	u8  n_counter = 0;
	u8  quality;
...
// Added for DL3
                        if(DeviceMode == MODE_FIRE){
                          ...
                          if (strob_pulse_timer == 0) {
                            fStartPulse = 1;                        
                            strob_pulse_timer = 120;
                          }
                          else{
                            strob_pulse_timer--;
                          }
...
//end DL3
			...
			if (DeviceMode == MODE_NORM) {
				light_timer = 1;				// Value - the light power
				flash_period_timer = 120;		// Flash after 1.2 sec
//				flash_period_timer = 200;		// Flash after 2 sec
			}
			//
...
void JP1_Define(void) {
	...
	temp = ADC_Measure_Simple(ADC_BUT, buf, 4);
